#### Preamble ####
# Purpose: Prepare and clean the survey data downloaded from nationscape 2020 june 25th survey.
# Author: Maxwell Garrett
# Data: 2 November 2020
# Contact: maxwell.garrett@mail.utoronto.ca
# License: MIT
# Pre-requisites: 
# - Need to have downloaded the data from X and save the folder that you're 
# interested in to inputs/data 
# - Don't forget to gitignore it!


#### Workspace setup ####
library(haven)
library(tidyverse)
setwd("C:/Users/Max/OneDrive - University of Toronto/4th Year/Semester 1/STA304/Problem Set 3")
# Read in the raw data (You might need to change this if you use a different dataset)
raw_data <- read_dta("inputs/ns20200625/ns20200625.dta")
# Add the labels
raw_data <- labelled::to_factor(raw_data)
# Just keep some variables
reduced_data <- 
  raw_data %>% 
  select(#interest,
         #registration,
         #vote_2016,
         #vote_intention,
         vote_2020,
         #ideo5,
         #employment,
         #foreign_born,
         gender,
         #census_region,
         #hispanic,
         race_ethnicity,
         #household_income,
         #education,
         state,
         #congress_district,
         age)


#### What else???? ####
# Maybe make some age-groups?
# Maybe check the values?
# Is vote a binary? If not, what are you going to do?

reduced_data<-
  reduced_data %>%
  mutate(vote_trump = 
           ifelse(vote_2020=="Donald Trump", 1, 0))

reduced_data<-
  reduced_data %>%
  mutate(vote_biden = 
           ifelse(vote_2020=="Joe Biden", 1, 0))

reduced_data <- reduced_data %>%
  mutate(white = as.character(race_ethnicity)) %>%
  mutate(white = replace(white, white != "White", "non-white")) %>%
  mutate(white = replace(white, white == "White", "white")) %>%
  mutate(white = as.factor(white))

# Saving the survey/sample data as a csv file in my
# working directory
write_csv(reduced_data, "outputs/survey_data.csv")

